...
const drawBigCircle = canvas.drawBigCircle,
      drawLittleCircle = canvas.drawLittleCircle
// 
...
Page({
  ...
  onShow: function () {
    ...
    wx.getSystemInfo({
      success(res) {
        ...
        that.timeCanvas = ctx
        drawBigCircle(ctx, canvasW, circlePos, circleR)
        that.timeCount()
      }
    })
  },
  ...
  timeCount() {
    interval = setInterval(() => {
      ...
      if (this.data.pause) {
        drawLittleCircle(this.timeCanvas, pi * (addTmp + add), 0, canvasW, circlePos, circleR)
        this.setData({
          pause: false
        })
      }
      drawLittleCircle(this.timeCanvas, pi * (addTmp + add), pi * addTmp, canvasW, circlePos, circleR)
